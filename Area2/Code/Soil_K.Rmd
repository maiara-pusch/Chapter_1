...
Reading soil data
```{r}
dados = read.csv("../Data/Soil/soil_ipiranga_grade.csv")
dados=na.omit(dados)
names(dados)
```
```{r}
...
```{r}
dados= dados %>% dplyr::select(-X, -Sandy, -Silt, -Clay, -MO,-P,-CTC,-BS,-TPSF,-optional)
```   
...
<!-- dados = cbind(dados, raster::extract(variaveis_auxiliares, dados)) -->
<!-- dados -->
<!-- ``` -->
...
ku= georob::georob(K ~ CH_1 + elevacao + talhoes, data = dados,  locations = ~ x + y,  variogram.model = "RMmatern",
...
library(dplyr)
names(dados)
dat = dplyr::select(as.data.frame(dados),-b3,-b4,-b5, -catch_area, -sw1, -sw2, - CH_0.5m, - coordx, -coordy, residuo.aic, residuo.ku)
...
grid$talhoes= as.factor(grid$talhoes)
