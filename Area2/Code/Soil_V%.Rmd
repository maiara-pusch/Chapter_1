...
```{r}
#load("RData/analise.bs.ipiranga.RData")
```

installation of packages
```{r}
install.packages("pacman")
pacman::p_load(georob, dplyr, tidyverse,pedometrics,raster,gstat,googlesheets,mapview,corrr,sf,robustbase,reshape,MASS,ggplot2,colormap,seewave,rgdal,caret,car)
```
     
 Reading soil data     
```{r}
dados = read.csv("../Data/Soil/data_V.csv")
dados=na.omit(dados)
names(dados)
```
...
```{r}
dados= dados %>% dplyr::select( x, y, BS)
```  
...
crs(dados)= crs(poly)
#spplot(dat, zcol="CTC") 
mapview(poly, alpha.regions = 0.01, color = "red", map.types = c("Esri.WorldImagery")) + mapview(dados, zcol="BS", legend=T)
```

```{r}
par(mfcol=c(2,2))
boxplot(dados$BS, horizontal=TRUE, outline=TRUE)
hist(dados$BS, col= "gray", xlab="BS", main=" BS")
#hist(log(dados$BS), col= "gray", xlab="Potássio", main="log(BS)")
#qqnorm(dados$BS)
```
...
```
 ...
 Check the relationship of soil types with BS
```{r}
np= plyr::count(dados$TPS)

ggplot(as.data.frame(dados), aes(TPS, BS)) +
  ylab("V (%)")+
  xlab("Fields")+
  geom_boxplot(outlier.colour = "red") + 
  annotate("text",x=np$x, y=90, label = np$freq, colour= "red", size=4.5)+
theme_bw(base_size = 15)
``` 

Check the relationship of the butchers with the BS
```{r}
np= plyr::count(dados$talhoes)

ggplot(as.data.frame(dados), aes(talhoes, BS)) +
  ylab("V (%)")+
  xlab("Fields")+
  geom_boxplot(outlier.colour = "red") + 
  annotate("text",x=np$x, y=90, label = np$freq, colour= "red", size=4.5)+
...
```{r}
reg.cov <- lm(BS ~ ., data = dplyr::select(as.data.frame(dados), -CH_1,-b4,-b5, -catch_area, -sw1, -sw2, - CH_0.5m, - coordx, -coordy))
...
```{r}
plot(ku.sv <-sample.variogram(residuals(reg.aic),locations = dados@coords, lag.dist.def= seq(0,800, by=100), estimador="qn"), type="p", annotate=T, main="Resíduos da regressão ~ cov ");grid()
```

```{r}
ku= georob::georob(BS ~ slope + SPI + talhoes + TWI + coordy, data = dados,  locations = ~ coordx + coordy,variogram.model = "RMmatern", param=c(variance=10, nugget=15, scale= 100, nu=0.5), fit.param = default.fit.param(variance=T, nugget = T, scale=T, nu=F), psi.func = c("huber"))

ku.sph= georob::georob(BS ~ slope + SPI + talhoes + TWI + coordy, data = dados,  locations = ~ x + y,variogram.model = "RMspheric", param=c(variance=10, nugget=10, scale= 100), fit.param = default.fit.param(variance=T, nugget = T, scale=T), psi.func = c("huber"))

ku.exp= georob::georob(BS ~ slope + SPI + talhoes + TWI + coordy, data = dados,  locations = ~ x + y,variogram.model = "RMexp", param=c(variance=20, nugget=10, scale= 250), fit.param = default.fit.param(variance=T, nugget = T, scale=T), psi.func = c("huber"))
```
Does not converge 
```{r}
#summary(ku)
...
# prfl.ku.exp <- profilelogLik(ku.exp ,values=data.frame(scale=seq(100, 800, by=25)))
# plot(loglik~scale, prfl.ku.exp, type="l", main="ku.exp")
...
# cv.ku.sph =georob::cv(ku.sph, seed= 42, nset =95, ncores=6)
```
...
```{r}
reg.xy= lm(BS ~ coordx+ coordy,dados)
plot(ku.sv <-sample.variogram(residuals(reg.xy),locations = dados@coords, lag.dist.def= seq(0,800, by=60), estimador="qn"), type="p", annotate=T, main="Resíduos da regressão ~ x+y ")

reg.xy= lm(BS ~ I(coordx^2)+I(coordy^2)+I(coordx*coordy),dados)
plot(ku.sv <-sample.variogram(residuals(reg.xy),locations = dados@coords, lag.dist.def= seq(0,800, by=60), estimador="qn"), type="p", annotate=T, main="Resíduos da regressão ~ x+y 2°ordem ")
```


```{r}
ku.xy = georob::georob(BS ~ coordx + coordy, data = dados,  locations = ~ x + y, variogram.model = "RMmatern", param=c(variance=30, nugget=10, scale= 300, nu=0.5), fit.param = default.fit.param(variance=T, nugget = T, scale=T, nu=F), psi.func = c("huber"), verbose=1)

ku.xy.sph = georob::georob(BS ~ coordx + coordy, data = dados,  locations = ~ x + y, variogram.model = "RMspheric", param=c(variance=30, nugget=10, scale= 300), fit.param = default.fit.param(variance=T, nugget = T, scale=T),psi.func = c("huber"), verbose=1)

ku.xy.exp = georob::georob(BS ~ coordx + coordy, data = dados,  locations = ~ x + y, variogram.model = "RMexp", param=c(variance=30, nugget=10, scale= 300), fit.param = default.fit.param(variance=T, nugget = T, scale=T),psi.func = c("huber"), verbose=1)
```
...
```{r} 
reg.kri= lm(BS ~1, dados)
plot(kri.sv <-sample.variogram(residuals(reg.kri),locations = dados@coords, lag.dist.def= seq(0,800, by=100), estimador="qn"), type="p", annotate=T, main="regressão ~ BS")
```
...
```{#r fig.align='center', fig.cap="Fig 11.Variograma amostral residual dos dados para a regressão linear de saturação por base (pontos) e função Matern ajustada (linha)"}
...
```{#r, fig.align='center', fig.cap="Fig 12 .Dados preditos versus dados observados"}
...
```{r}
#print(plot(kri, lag.dist.def=seq(0,800, by=100), max.lag=1100, estimator="qn", xlab ="Distance", ylab = "Semivariance", type="p"));grid()
print(plot(ku.xy, lag.dist.def=seq(0,600, by=100), estimator="qn", xlab ="Distance", ylab = "Semivariance", type="p", cex.lab=1.5, cex.axis=1.5, col="blue"));grid()
...
title("V (%) - Area 2")
legend("bottomright", lt=1, col=c("blue"), legend = c("Robust Universal Kriging ~ x + y"), cex=1.2)
```

Cross-validation (LOOCV) deterministic model IDW
```{r}
cv.idw=gstat::krige.cv(BS ~ 1, dados, nfold = 96)
cv.idw=as.data.frame(cv.idw)
statistics.idw <- c(
...
```{r}
dat = dplyr::select(as.data.frame(dados), -CH_1,-b3,-b4,-b5, -catch_area, -sw1, -sw2, - CH_0.5m, - coordx)
```

```{r}
model= MASS::rlm(BS ~  slope + SPI + talhoes + TWI + coordy, data=dat, psi = psi.huber, k2= 2, escale.est= "Huber")
...
```{r}
statistics.idw;statistics.xy;statistics.predict; 
```
...
map.idw= gstat::idw(BS ~ 1, dados, grid_interpolacao)
...
ggplot(df1, aes(x= value, y= Methods, fill= Methods) )+
  ...
  labs(title = "V (%) - Area 2") +
  scale_fill_manual(values=c("#440154FF", "#414487FF", "#22A884FF", "#FDE725FF"))+
...
save.image("RData/analise.bs.ipiranga.RData")
```
